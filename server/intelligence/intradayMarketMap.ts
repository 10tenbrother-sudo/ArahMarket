...
export class IntradayMarketMapEngine {
  ...
  public static getIntradayMarketMap(): IntradayAssetBias[] {
    ...
    const targetSymbols: Array<{
      symbol: string;
      displayName: string;
      assetType: 'COMMODITY' | 'CRYPTO' | 'INDEX' | 'FOREX';
      tvSymbol: string;
      tvUrl?: string;
    }> = [
  ...
  public static getTodayKeyCatalysts(): TodayCatalyst[] {
    ...
    // Define Today's trading session window (last 16h to next 24h)
    const windowStart = nowMs - 16 * 3600000;
    const windowEnd = nowMs + 24 * 3600000;

    const todayEvents = macroEvents.filter(e => {
